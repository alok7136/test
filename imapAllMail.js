...
imap.once("ready", function () {
  ...
  imap.openBox("INBOX", false, function (err, box) {
    ...
    if (err) throw err;
    try {
      imap.search(["UNSEEN",["1:20"]], function (err, results) {
        console.log(results);
        ...
        if (err) throw err;
        try {
          var g = imap.fetch(results, { request: { headers: ['from', 'to', 'subject', 'date'] } });
          ...
          g.on("result", function (msg, seqno) {
            var prefix = "(#" + seqno + ") ";
            msg.on("body", function (stream, info) {
              var buffer = "";
